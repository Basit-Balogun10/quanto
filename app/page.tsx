...
export default function Home() {
  ...
  const currentPersona = useMemo(() => {
    ...
    const foundPersona = Object.values(personas).find((p) => p.id.includes(userId.toLowerCase()))
  ...
  const screenContent = useMemo(() => {
    switch (currentScreen) {
      ...
      case "insights":
        return <InsightsScreen persona={currentPersona} onMarkAllRead={() => setUnreadInsights(0)} />
      case "settings":
